...
const int VIBRATION_TRIGGER_COUNT = 3;
const unsigned long VIBRATE_ON_MS = 600;
const unsigned long VIBRATE_OFF_MS = 300;
const unsigned long VIBRATE_PATTERN_MS = (VIBRATE_ON_MS * 3UL) + (VIBRATE_OFF_MS * 2UL);
...
struct TankConfig {
  ...
  float lastSaneDistance = -1.0;
  unsigned long vibrationStartTime = 0;
  bool isVibrating = false;

  // --- FILL PROGRESS TRACKING (Overflow Prevention) ---
  int levelAtLastProgressCheck = 0;
  unsigned long lastProgressCheckTime = 0;
  int progressCheckMinutes = 10;
};
...
<style>
  ...
  <div class="card">
    ...
    <form action="/save" method="POST">
      ...
      <div class="lbl-wrap"><label>Fill Progress Timeout</label><span class="range">5 - 30 m</span></div><select name="pLckM" id="pLckM_s"></select>
...
<script>
  ...
  window.onload = () => {
    fetch('/config').then(r=>r.json()).then(c=>{
       ...
       let pL=""; for(let i of [5,10,15,20,25,30]) pL+=`<option value="${i}" ${c.pLckM==i?"selected":""}>${i} Minutes</option>`; document.getElementById('pLckM_s').innerHTML = pL;
...
void startSensorVibration() {
  if (tankConfig.isVibrating) return;
  Serial.println("[ACTION] Starting 3-pulse vibration pattern to clear sensor...");
  tankConfig.vibrationStartTime = millis();
  tankConfig.isVibrating = true;
  digitalWrite(VIBRATOR_PIN, HIGH);
}

void updateSensorVibration() {
  if (!tankConfig.isVibrating) return;

  unsigned long elapsed = millis() - tankConfig.vibrationStartTime;
  if (elapsed >= VIBRATE_PATTERN_MS) {
    digitalWrite(VIBRATOR_PIN, LOW);
    tankConfig.isVibrating = false;
    tankConfig.upperInvalidCount = 0;
    Serial.println("[ACTION] Vibrator pattern complete. Settling before next ultrasonic read...");
    return;
  }

  unsigned long cycle = elapsed % (VIBRATE_ON_MS + VIBRATE_OFF_MS);
  digitalWrite(VIBRATOR_PIN, cycle < VIBRATE_ON_MS ? HIGH : LOW);
}
...
void monitorSensors() {
  ...
  if (!upperSensor.isBusy() && now - lastScan >= ULTRASONIC_INTERVAL) {
    ...
    if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(50))) {
      if (dist > 0) {
        // --- DELTA CHECK LOGIC ---
        const float MAX_CHANGE = 5.0f;
        bool isSane = false;

        if (!tankConfig.firstReadingDone) {
          tankConfig.lastSaneDistance = dist;
          isSane = true;
        } else {
          float delta = abs(dist - tankConfig.lastSaneDistance);
          if (delta < MAX_CHANGE) {
            isSane = true;
            tankConfig.lastSaneDistance = dist;
          }
        ...
        if (isSane) {
          ...
          digitalWrite(VIBRATOR_PIN, LOW);  // Ensure vibrator is OFF if reading is good
          tankConfig.isVibrating = false;

          // TILT CORRECTION: Multiply by cos(15 deg) = 0.966
          float correctedDist = dist * 0.966f;

          if (!tankConfig.firstReadingDone) {
            tankConfig.upperDistance = correctedDist;
            tankConfig.firstReadingDone = true;
          } else {
            // Trust new data 25%, keep old data 75% for much smoother display
            tankConfig.upperDistance = (0.25f * correctedDist) + (0.75f * tankConfig.upperDistance);
          }
          ...
          // --- DATA IS BAD (Water drop suspected) ---
          tankConfig.upperInvalidCount++;
          Serial.println("[SENSOR] Delta Check Rejected: Water drop suspected.");

          if (tankConfig.upperInvalidCount >= VIBRATION_TRIGGER_COUNT) startSensorVibration();
        }
      } else {
        tankConfig.upperInvalidCount++;
        if (tankConfig.upperInvalidCount >= VIBRATION_TRIGGER_COUNT) startSensorVibration();
...
void setup() {
  ...
  tankConfig.progressCheckMinutes = preferences.getInt("pLckM", tankConfig.progressCheckMinutes);
...
String getStartBlockReason() {
  if (pumpConfig.fillProgressLock) return "Safety Lock: Level stagnant. Check sensor!";
...
String processManualToggle() {
  ...
  if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(500))) {
    ...
    pumpConfig.fillProgressLock = false;
...
void updatePumpLogic() {
  ...
  if (msConfig.sysRole == 2 && pumpConfig.motorStatus == 1) {
    ...
    if (masterIsOnline) {
      ...
      if (!masterIsSafe) {
        ...
        Serial.println("[SLAVE] Master Not Ready. Sequential Stop triggered.");
        saveMotorStatus();
        triggerPublish = true;
      }
    }
  ...
  // --- DELTA CHECK SELF-HEALING ---
  static unsigned long errorDetectedTime = 0;
  if (sensorError) {
    if (errorDetectedTime == 0) errorDetectedTime = currentMillis;
    if (currentMillis - errorDetectedTime > 120000UL) {
      tankConfig.lastSaneDistance = -1.0;
      tankConfig.firstReadingDone = false;
      tankConfig.upperInvalidCount = 0;
      tankConfig.errorAck = false;
      errorDetectedTime = 0;
      Serial.println("[SENSOR] Full state reset for auto-recovery.");
    }
  } else {
    errorDetectedTime = 0;
  }

  // --- 3. FILL PROGRESS / STUCK SENSOR PROTECTION ---
  if (pumpConfig.motorStatus == 1 && currentState == PumpState::PUMPING) {
    if (tankConfig.lastProgressCheckTime == 0) {
      tankConfig.lastProgressCheckTime = currentMillis;
      tankConfig.levelAtLastProgressCheck = tankConfig.displayUpperPercentage;
    }
    if (currentMillis - tankConfig.lastProgressCheckTime >= (unsigned long)tankConfig.progressCheckMinutes * 60000UL) {
      if (tankConfig.displayUpperPercentage <= tankConfig.levelAtLastProgressCheck) {

        Serial.println("[SAFETY] Hard Lock: No water rise in " + String(tankConfig.progressCheckMinutes) + " mins.");

        pumpConfig.motorStatus = 0;  // Stop Motor
        pumpConfig.manualOverride = false;
        pumpConfig.fillProgressLock = true;  // <--- LOCK THE SYSTEM
    ...
    tankConfig.lastProgressCheckTime = 0;
  }
  ...
  if (!pumpConfig.fillProgressLock && pumpConfig.motorStatus == 0 && tankConfig.displayUpperPercentage <= tankConfig.LOW_THRESHOLD && voltageConfig.status == 1 && dryRunConfig.error == 0 && !sensorError && !coolDownConfig.isResting && !currentDndActive && getStartBlockReason() == "") {
    pumpConfig.motorStatus = 1;
  ...
  // --- 8. COOL-DOWN & VOLTAGE PROTECTION ---
  ...
  if (coolDownConfig.isResting) {
    if (currentMillis - coolDownConfig.restStartTime >= (unsigned long)coolDownConfig.restMinutes * 60000UL) {
      ...
      if (pumpConfig.wasRunningBeforeCoolDown && getStartBlockReason() == "" && voltageConfig.status == 1 && (!currentDndActive || pumpConfig.manualOverride)) {
        pumpConfig.motorStatus = 1;
        coolDownConfig.runStartTime = currentMillis;
      }
  ...
  if (voltAbnormal) {
    ...
    if (currentMillis - voltageConfig.lastCheck >= 1000) {
      ...
      if (voltageConfig.waitSeconds <= 0) {
        ...
        if (pumpConfig.wasRunningBeforeVoltageError && getStartBlockReason() == "" && !sensorError && !coolDownConfig.isResting && (!currentDndActive || pumpConfig.manualOverride)) {
          pumpConfig.motorStatus = 1;
          coolDownConfig.runStartTime = currentMillis;
        }
...
void updateLCD() {
  ...
  } else if (pumpConfig.fillProgressLock) info = " TANK LOCK ";
...
void handleConfig() {
  if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(500))) {
    ...
    doc["pLckM"] = tankConfig.progressCheckMinutes;
...
void handleSave() {
  ...
  if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(3000))) {
    ...
    if (server.hasArg("pLckM")) {
      tankConfig.progressCheckMinutes = server.arg("pLckM").toInt();
      preferences.putInt("pLckM", tankConfig.progressCheckMinutes);
    }
...
String generateStatusJson() {
  ...
  else if (pumpConfig.fillProgressLock) info = "STAGNANT_LOCKED!";
  ...
  if (doc.containsKey("toggle")) {
    ...
    if (doc.containsKey("pin") && String(doc["pin"].as<const char*>()) == devicePin) {
      ...
      if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(3000))) {
        ...
        if (doc.containsKey("pLckM")) {
          tankConfig.progressCheckMinutes = doc["pLckM"].as<int>();
          preferences.putInt("pLckM", tankConfig.progressCheckMinutes);
        }

...
<style>
  ...
  <div class="card">
    <div style="font-size:0.8rem;text-align:left;margin-bottom:10px;"><span id="dot" class="conn-dot"></span><span id="cStat" style="color:#aaa;">Device: Online</span></div>
...
<script>
  ...
  const upd = () => { return fetch('/status').then(r=>r.json()).then(d=>{
    if(d.lang == 1) { document.body.classList.add('lang-mm'); applyDict(); } else document.body.classList.remove('lang-mm');
      ...
      setBadge('vstat', vS, (d.vStat=="NORMAL") ? 'success' : (d.vStat=="DELAY" ? 'warning' : 'danger'));
      setBadge('state', pS, (d.pStat=="ON") ? 'success' : (d.pStat=="DRY ALRM"||d.pStat=="LOCKED" ? 'danger' : 'secondary'));
      
      let iType = 'info';
      if (d.info.includes("ALARM") || d.info.includes("ERROR")) iType = 'danger';
      else if (d.info.includes("WAITING") || d.info.includes("LOCKED") || d.info.includes("COOLING") || d.info.includes("VENTING")) iType = 'warning';
      else if (d.info.includes("FLOW_DETECTED")) iType = 'success';
...
void updatePumpLogic() {
  ...
  // Slug Window: Even if the sensor stops seeing water briefly (air bubbles),
  // we treat flow as "Active" for 60s to prevent accidental Dry-Run trips.
  ...
  // --- 2. CHECK DND STATUS (STABILIZED WITH OFFLINE FALLBACK) ---
  if (scheduleConfig.enabled) {
    struct tm timeinfo;
    bool timeIsReady = false;

    // Try to get time, and ensure NTP has actually synced (Year > 2020)
    if (getLocalTime(&timeinfo, 10) && timeinfo.tm_year > 120) {
        ...
        if (scheduleConfig.dndStart > scheduleConfig.dndEnd) {
          ...
          // Standard case (e.g., 08:00 to 17:00)
          if (hour >= scheduleConfig.dndStart && hour < scheduleConfig.dndEnd) isInsideDndWindow = true;
        }
    ...
    // Fix 3: FALLBACK - If Internet/NTP fails, don't lock the pump
    if (!timeIsReady) {
      if (currentDndActive == true) {
        currentDndActive = false; // Disable DND so pump can still work offline
        triggerPublish = true;
        Serial.println("[TIME] NTP lost. DND disabled for safety.");
      }
    }
  } else {
  ...
  // AUTO START: If tank is low, turn on

...
String lastLoggedState = "";  // Prevents duplicate logs
...
<style>
  ...
  <div class="card">
    ...
    <form action="/save" method="POST">
      ...
      <hr>
<button type="button" class="btn btn-purple" style="margin-bottom:10px; padding:10px;" onclick="unlockFactory()">🛠️ Factory Mode</button>
<div id="factorySec" style="display:none; background:rgba(255,255,255,0.05); padding:15px; border-radius:12px; border:1px solid rgba(255,255,255,0.1);">
    <h3 style="color:#03ef; text-align:center;">Cloud Server Config</h3>
    <div class="lbl-wrap"><label>MQTT Server URL</label></div><input type="text" name="mqSrv" id="mqSrv_s">
    <div class="lbl-wrap"><label>MQTT Port</label></div><input type="number" name="mqPort" id="mqPort_s">
    <div class="lbl-wrap"><label>MQTT Username</label></div><input type="text" name="mqUser" id="mqUser_s">
    <div class="lbl-wrap"><label>MQTT Password</label></div><input type="password" name="mqPass" id="mqPass_s">
</div>
...
<script>
  ...
  function unlockFactory(){
    let p = prompt("Enter Factory Admin Key:");
    if(p === "AMK_ADMIN_2026"){
        document.getElementById('factorySec').style.display='block';
  ...
  window.onload = () => {
    fetch('/config').then(r=>r.json()).then(c=>{
       ...
       document.getElementById('mqSrv_s').value = c.mqSrv || "";
document.getElementById('mqPort_s').value = c.mqPort || 8883;
document.getElementById('mqUser_s').value = c.mqUser || "";
document.getElementById('mqPass_s').value = c.mqPass || "";
       toggleVDly();
       if(c.lang==1) {
...
void setup() {
  ...
  mqtt_server = preferences.getString("mqSrv", "210195b635414206adcd944325fe6f59.s1.eu.hivemq.cloud");
  mqtt_port = preferences.getInt("mqPort", 8883);
  mqtt_user = preferences.getString("mqUser", "Smart_Pump");
  mqtt_pass = preferences.getString("mqPass", "Sm@rt_Pump_2026");
  ...
  if (!LittleFS.begin(true)) {
    ...
    // Log the power on event
    logEvent("SYS: Power On");
  }
...
// 4. NORMAL TOGGLE (Start or Stop)
    ...
    if (wantsToStart) {
      ...
      dryRunConfig.waitSeconds = dryRunConfig.WAIT_SECONDS_SET;
      logEvent("SYS: Manual Start"); // Specific log
...
void updatePumpLogic() {
  ...
  if (currentCondition != lastStableCondition) {
      if (currentCondition == "NORMAL") logMessage = "SYS: Guards OK";
      ...
      else if (currentCondition == "SENSOR_ERR") logMessage = "ERR: Sensor Fail";
      else if (currentCondition == "DRY_RUN") logMessage = "ALR: Dry Run";
      else if (currentCondition == "RESTING") logMessage = "SYS: Cooldown";
...
void handleConfig() {
  if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(500))) {
    ...
    doc["mqPort"] = mqtt_port;
    doc["mqUser"] = mqtt_user;
    doc["mqPass"] = mqtt_pass;
    doc["pin"] = devicePin;
    doc["lang"] = sysLang;
...
void handleSave() {
  ...
  if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(3000))) {
    ...
    if (server.hasArg("mqSrv")) preferences.putString("mqSrv", server.arg("mqSrv"));
    if (server.hasArg("mqPort")) preferences.putInt("mqPort", server.arg("mqPort").toInt());
    if (server.hasArg("mqUser")) preferences.putString("mqUser", server.arg("mqUser"));
    if (server.hasArg("mqPass")) preferences.putString("mqPass", server.arg("mqPass"));
...
String generateStatusJson() {
  ...
  File logFile = LittleFS.open("/events.txt", "r");
  if (logFile) {
    String tempLogs = "";
    if (logFile.size() > 400) logFile.seek(logFile.size() - 400);  // Only send last 400 chars
    while (logFile.available()) tempLogs += (char)logFile.read();
    doc["logs"] = tempLogs;
    logFile.close();
  } else {
    doc["logs"] = "No history yet.";
  }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc.containsKey("clearLog")) {
    if (doc.containsKey("pin") && String(doc["pin"].as<const char*>()) == devicePin || String(doc["pin"].as<const char*>()) == ADMIN_MASTER_KEY) {
        LittleFS.remove("/events.txt");
        Serial.println("Event History Cleared");
        logEvent("SYS: History Reset"); // Start fresh with the new date format
        pendingMqttPublish = true; 
    }
    return;
  }
  ...
  if (doc.containsKey("toggle")) {
    ...
    if (doc.containsKey("pin") && String(doc["pin"].as<const char*>()) == devicePin) {
      ...
      if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(3000))) {
        ...
        if (doc.containsKey("mqSrv")) preferences.putString("mqSrv", doc["mqSrv"].as<String>());
        if (doc.containsKey("mqPort")) preferences.putInt("mqPort", doc["mqPort"].as<int>());
        if (doc.containsKey("mqUser")) preferences.putString("mqUser", doc["mqUser"].as<String>());
        if (doc.containsKey("mqPass")) preferences.putString("mqPass", doc["mqPass"].as<String>());
